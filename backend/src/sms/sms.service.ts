...
export class SmsService {
  ...
  async sendSms(
    ...
    try {
      ...
      if (!formExist) {
        return { ok: false, error: '폼이 존재하지 않습니다.' };
      }
      ...
      if (!question) {
        return { ok: false, error: '폼 내부에 존재하지 않는 질문입니다.' };
      }

      if (question.kind !== QuestionType.Personal) {
        return { ok: false, error: '개인정보 질문이 아닙니다.' };
      }
      ...
      if (submissionIds.length !== form.length) {
        return {
          ok: false,
          error: '답변이 폼 안에 존재하지 않거나 질문이 존재하지 않습니다.',
        };
      }
      ...
      if (res.result_code == 1) {
        this.addHistoryInDB(user, msg, formId, submissionIds);
